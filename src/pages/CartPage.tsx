      ...
      <div className="min-h-screen bg-[#0f172a] flex flex-col items-center justify-center text-white p-4">
        <div className="max-w-md text-center space-y-6">
          ...
          <div className="flex gap-4 justify-center">
            <Link to="/auth">
              <Button size="lg" className="bg-orange-600 hover:bg-orange-700 font-bold px-8">
                Sign In
              </Button>
            </Link>
            <Link to="/design">
            ...
            <div className="lg:col-span-2 space-y-8">
                            ...
                            <div className="flex-1 min-w-0 flex flex-col justify-between min-h-[120px]">
                              <div>
                                <div className="flex justify-between items-start">
                                    <h3 className="font-bold text-lg text-slate-200 truncate pr-4">{item.title}</h3>
                                ...
                                <div className="flex flex-wrap gap-2 mt-1 text-sm text-slate-400">
                                  <span className="px-2 py-0.5 bg-slate-900 rounded border border-white/5">{item.variant.size}</span>
                                  <span className="px-2 py-0.5 bg-slate-900 rounded border border-white/5">{item.variant.color}</span>
                                </div>
                              </div>
                              ...
                              <div className="flex flex-wrap items-end justify-between gap-4 mt-4">
                                 ...
                                 <div className="flex items-center gap-3 bg-slate-900 rounded-lg p-1 border border-white/10">
                                   <button 
                                      ...
                                      className="p-1.5 hover:text-white text-slate-400 disabled:opacity-50 transition-colors"
              ...
              {!isSavedEmpty && (
                 <div className="mt-12 pt-8 border-t border-white/10">
                    ...
                    <div className="space-y-4">
                      {savedItems.map((item) => (
                         <div key={item.id} className="flex gap-4 p-4 rounded-xl border border-dashed border-white/10 bg-slate-900/20 hover:bg-slate-900/40 transition-colors">
                            {/* Thumbnail */}
                            <div className="h-20 w-20 bg-white rounded-lg p-2 flex-shrink-0 grayscale opacity-80 hover:grayscale-0 hover:opacity-100 transition-all">
                               <img src={item.thumbnail} alt={item.title} className="h-full w-full object-contain" />
                            </div>
                            ...
                            <div className="flex-1 flex flex-col justify-between">
                               <div>
                                  ...
                                  <p className="text-sm text-slate-500">{item.variant.size} / {item.variant.color} • <span className="text-slate-300">₹{item.price}</span></p>
                                </div>
